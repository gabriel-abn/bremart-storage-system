...
class RegisterManyProductsUseCase(RegisterManyProducts):
    def __init__(
        self,
        ...
        for product in products:
            ...
            if not distributor:
                raise ApplicationError(
                    "Distributor not found", "RegisterProductUseCase"
                )

            if product["product_type"] not in self.get_product_types.execute():
                raise ApplicationError(
                    "Invalid product type.", "RegisterProductUseCase"
                )
            ...
            ids.append(self.crypter.encrypt(save["id"]))
